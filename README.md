...
Inside a club/day, students are grouped into one subsection per class
(sorted naturally, e.g. 0А, 0В, 1А, 2А, 2В…), each with its own heading
showing the class name and a properly pluralized count (KZ/RU/EN). Search
and the class filter both operate across these grouped subsections — the
class filter isolates one class's group, and search matches by name or
class within whatever is currently shown.
